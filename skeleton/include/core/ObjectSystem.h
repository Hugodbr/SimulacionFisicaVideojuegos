...
protected:
	...
	std::vector<std::unique_ptr<ObjectSystem>> _subSystems;
...
public:
	ObjectSystem();
	virtual ~ObjectSystem() = default; // smart pointers will auto-clean
	...
	virtual void onRender(const glm::mat4& modelViewMat) = 0; // Hook for derived classes
	...
	virtual void setRenderableForObject(PhysicalObject& object);
...
protected:
	...
	virtual bool mustKillObject(const PhysicalObject& object, const ObjectGenerator& generator) const;

	virtual bool mustSpawnObject(double deltaTime, const ObjectGenerator& generator) const;
	...
	void registerSubSystem(std::unique_ptr<ObjectSystem> subSystem);
